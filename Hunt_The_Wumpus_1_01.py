...
def main():
    ...
    pit1 = random.randint(1,16)
    pit2 = random.randint(1,16)
    if pit1 == wumpusRoom:
        pit1 = random.randint(1,16)
    if pit2 == wumpusRoom:
        pit2 = random.randint(1,16)
    if pit1 == pit2:
        pit2 = random.randint(1,16)
    #Reset player location so they have only a (1/16^2) *3 (about 1 in 133) chance to spawn in the same room as the wumpus or a pit. 
    #The cave also has about a 1 in 400 chance to generate with only one pit.
    if playerLocation == wumpusRoom or playerLocation == pit1 or playerLocation == pit2:
        playerLocation = random.randint(1,16)
    crash = False
    #go to wumpus function to play
    wumpus(wumpusRoom,playerLocation,pit1,pit2,crash)
    
def wumpus(wumpusRoom,playerLocation,pit1,pit2,crash):
    crash = False
    while crash == False:
        ...
        if playerLocation == wumpusRoom:
            ...
            crash = True
            break
        #Check if player spawns in pit on rare off-chance
        ...
        if playerLocation == 1:
            ...
            #Tell user if wumpus is in room within shooting range and if pit is in neighbouring cave.
